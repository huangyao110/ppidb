...
conda install -c bioconda mmseqs2  # for similarity_split
```
...
ppidb is designed to work with a unified PPI parquet file (e.g. `ppi_final_uniprot_v6.parquet`) that aggregates interactions from multiple public databases.
...
seqs = SequenceFetcher().fetch(all_proteins, as_dict=True)
